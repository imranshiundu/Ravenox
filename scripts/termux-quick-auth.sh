...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME"@ravenox/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Ravenox Re-Auth" -c "After getting key, run: ssh $SERVER '~"@ravenox/scripts/mobile-reauth.sh'" --id.ravenox-auth
        ;;
    *)
