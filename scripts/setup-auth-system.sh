#!/bin/bash
# Setup Ravenox Auth Management System
...
echo "=== Ravenox Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., .ravenox-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Ravenox message: Send warning via Ravenox itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd.ravenox-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd.ravenox-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd.ravenox-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now.ravenox-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status.ravenox-auth-monitor.timer"
echo ""
echo "Quick commands:"
