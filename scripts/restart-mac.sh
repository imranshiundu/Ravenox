#!/usr/bin/env bash
# Reset Ravenox like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${RAVENOX_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Ravenox.app/Contents/MacOS/Ravenox"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Ravenox"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Ravenox"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Ravenox"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.ravenox.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}.ravenox-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${RAVENOX_RESTART_LOG:-/tmp.ravenox-restart.log}"
NO_SIGN=0
SIGN=0
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  RAVENOX_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node.ravenox.mjs daemon install --force --runtime node"
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.ravenox.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Ravenox instances"
kill_all.ravenox
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Ravenox"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Ravenox.app" ]]; then
    APP_BUNDLE="/Applications/Ravenox.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Ravenox.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Ravenox.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Ravenox.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set RAVENOX_APP_BUNDLE to your installed Ravenox.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node.ravenox.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node.ravenox.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".ravenox", .ravenox.json"), "utf8");
