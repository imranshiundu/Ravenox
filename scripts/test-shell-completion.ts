 ...
 * .ravenox update`. Use it to verify the completion installation flow
...
\.ravenox update\`, \.ravenox doctor\`, and \.ravenox onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Ravenox Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \.ravenox completion --install\` later to enable.`));
    return;
  }
