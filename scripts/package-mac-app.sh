...
# Build and bundle Ravenox into a minimal .app we can open.
# Outputs to dist/Ravenox.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Ravenox.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Ravenox"
BUNDLE_ID="${BUNDLE_ID:-ai.ravenox.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com.ravenox"@ravenox/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Ravenox/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :RavenoxBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :RavenoxGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Ravenox"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Ravenox"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Ravenox"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Ravenox" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Ravenox/Resources/Ravenox.icns" "$APP_ROOT/Contents/Resources/Ravenox.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Ravenox/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying RavenoxKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/RavenoxKit_RavenoxKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/RavenoxKit_RavenoxKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/RavenoxKit_RavenoxKit.bundle"
else
  echo "WARN: RavenoxKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Ravenox"
killall -q Ravenox 2>/dev/null || true
