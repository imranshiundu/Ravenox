...
INSTALL_URL="${RAVENOX_INSTALL_URL:-https:/.ravenox.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${RAVENOX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${RAVENOX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE=.ravenox"
PACKAGE_NAME="${RAVENOX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (RAVENOX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${RAVENOX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${RAVENOX_INSTALL_LATEST_OUT:-}"
fi
