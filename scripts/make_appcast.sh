...
ZIP=${1:?"Usage: $0 Ravenox-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com.ravenox.ravenox/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Ravenox-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com.ravenox.ravenox/releases/download/v${VERSION}/"}
