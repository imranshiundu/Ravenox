...
const shouldBuild = (deps) => {
  if (deps.env.RAVENOX_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.RAVENOX_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`.ravenox] ${message}\n`);
};

const runRavenox = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, [.ravenox.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
