...
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g.,.ravenox-alerts)
...
send_notification() {
    ...
    # Send via Ravenox if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use.ravenox
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Ravenox to $NOTIFY_PHONE..."
           .ravenox send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
