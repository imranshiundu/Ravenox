#!/usr/bin/env bash
# ClawDock - Docker helpers for Ravenox
# Inspired by Simon Willison's "Running Ravenox in Docker"
...
# Common paths to check for Ravenox
CLAWDOCK_COMMON_PATHS=(
  "${HOME}.ravenox"
  "${HOME}/workspace.ravenox"
  "${HOME}/projects.ravenox"
  "${HOME}/dev.ravenox"
  "${HOME}/code.ravenox"
  "${HOME}/src.ravenox"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Ravenox at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to.ravenox"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Ravenox not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com.ravenox.ravenox.git ~.ravenox"
    echo "  cd ~.ravenox && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^RAVENOX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d.ravenox-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart.ravenox-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f.ravenox-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec.ravenox-gateway \
    bash -c 'echo "alias.ravenox=\"..ravenox.mjs\"" > /tmp/.bashrc.ravenox && bash --rcfile /tmp/.bashrc.ravenox'
}
...
clawdock-cli() {
  _clawdock_compose run --rm.ravenox-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build.ravenox-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "RAVENOX_GATEWAY_TOKEN=$token".ravenox-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token".ravenox-gateway \
    bash -c '..ravenox.mjs config set gateway.remote.token "$TOKEN" && ..ravenox.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec.ravenox-gateway \
    bash -c "..ravenox.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart.ravenox-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm.ravenox-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec.ravenox-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd .ravenox config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Ravenox${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container .ravenox alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to.ravenox project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.ravenox)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd .ravenox channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd .ravenox status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.ravenox.ai${_CLR_RESET}"
  echo ""
}
