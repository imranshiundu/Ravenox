...
Inspired by Simon Willison's [Running Ravenox in Docker](https://til.simonwillison.net/llms.ravenox-docker).
...
On first command, ClawDock auto-detects your Ravenox directory:

- Checks common paths (`~.ravenox`, `~/workspace.ravenox`, etc.)
...
| `clawdock-cli <command>`  | Run Ravenox CLI commands                      |
...
| `clawdock-cd`        | Jump to the Ravenox project directory    |
| `clawdock-config`    | Open the Ravenox config directory        |
...
```bash
ravenox channels login --channel whatsapp --verbose
```
...
2. Configure it in the Ravenox config
...
- Bash or Zsh shell
- Ravenox project (from `docker-setup.sh`)
