...
IMAGE_NAME=.ravenox-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME=.ravenox-net-e2e-$$"
GW_NAME=.ravenox-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "RAVENOX_GATEWAY_TOKEN=$TOKEN" \
	  -e "RAVENOX_SKIP_CHANNELS=1" \
	  -e "RAVENOX_SKIP_GMAIL_WATCHER=1" \
	  -e "RAVENOX_SKIP_CRON=1" \
	  -e "RAVENOX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
