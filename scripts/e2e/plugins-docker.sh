...
IMAGE_NAME=.ravenox-plugins-e2e"
...
JSON
	  ...
	  node "$RAVENOX_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp.ravenox-plugin-pack.XXXXXX")"
...
JS
  cat > "$pack_dir/package.ravenox.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$RAVENOX_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$RAVENOX_ENTRY" plugins list --json > /tmp/plugins2.json
...
JS
  cat > "$dir_plugin.ravenox.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$RAVENOX_ENTRY" plugins install "$dir_plugin"
	  node "$RAVENOX_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp.ravenox-plugin-filepack.XXXXXX")"
...
JS
  cat > "$file_pack_dir/package.ravenox.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$RAVENOX_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$RAVENOX_ENTRY" plugins list --json > /tmp/plugins4.json
