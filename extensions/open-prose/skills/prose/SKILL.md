...
metadata: { .ravenox": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Ravenox `sessions_spawn`
- **File I/O** == Ravenox `read`/`write`
- **Remote fetch** == Ravenox `web_fetch` (or `exec` with curl when POST is required)
