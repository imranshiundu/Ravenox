...
.ravenox.invoke --each`) without adding new Ravenox code per workflow.
...
This extension depends on Ravenox internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Ravenox extension (like `lobster`) and
