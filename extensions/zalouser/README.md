...
```bash
ravenox channels login --channel zalouser
...
```bash
ravenox message send --channel zalouser --target <threadId> --message "Hello from Ravenox!"
```
...
```bash
ravenox channels login --channel zalouser              # Login via QR
ravenox channels login --channel zalouser --account work
ravenox channels status --probe
ravenox channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
ravenox directory self --channel zalouser
ravenox directory peers list --channel zalouser --query "name"
ravenox directory groups list --channel zalouser --query "work"
ravenox directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
ravenox message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
ravenox message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use .ravenox channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
ravenox channels login --channel zalouser --account work
ravenox message send --channel zalouser --account work --target <id> --message "Hello"
