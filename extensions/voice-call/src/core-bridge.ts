...
function resolveRavenoxRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.RAVENOX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set RAVENOX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveRavenoxRoot(), "dist", "extensionAPI.js");
