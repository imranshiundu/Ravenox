...
function resolveGatewayPort(cfg: RavenoxPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.RAVENOX_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: RavenoxPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: RavenoxPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Ravenox iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Ravenox iOS app:",
            "",
            "```",
            qrAscii,
            "```",
