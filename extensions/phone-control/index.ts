...
function normalizeDenyList(cfg: RavenoxPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: RavenoxPluginApi["config"]): string[] {
...
export default function register(api: RavenoxPluginApi) {
  ...
  const timerService: RavenoxPluginService = {
