...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as RavenoxConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as RavenoxConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as RavenoxConfig);
