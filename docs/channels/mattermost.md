---
summary: "Mattermost bot setup and Ravenox config"
read_when:
...
```bash
ravenox plugins install ./extensions/mattermost
```
...
4. Configure Ravenox and start the gateway.
...
- Approve via:
  - .ravenox pairing list mattermost`
  - .ravenox pairing approve mattermost <CODE>`
...
Use these target formats with .ravenox message send` or cron/webhooks:
