...
read_when:
  - You want Ravenox to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Ravenox to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (.ravenox onboard`) and .ravenox channels add` list optional channel plugins.
...
```bash
ravenox plugins install --link <path-to.ravenox>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": .ravenox@example.com",
        "lud16": .ravenox@example.com"
      }
    }
  }
}
