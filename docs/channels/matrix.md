...
Matrix is an open, decentralized messaging protocol. Ravenox connects as a Matrix **user**
...
```bash
ravenox plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: .ravenox plugins install .ravenox/matrix`
   - From a local checkout: .ravenox plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Ravenox calls the same
     login endpoint, stores the access token in `~/.ravenox/credentials/matrix/credentials.json`,
...
- On first connection, Ravenox requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Ravenox logs a warning.
...
`~/.ravenox/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - .ravenox pairing list matrix`
  - .ravenox pairing approve matrix <CODE>`
...
- On startup, Ravenox resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
