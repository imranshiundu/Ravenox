...
Tlon is a decentralized messenger built on Urbit. Ravenox connects to your Urbit ship and can
...
```bash
ravenox plugins install "@ravenox/tlon
```

Local checkout (when running from a git repo):

```bash
ravenox plugins install ./extensions/tlon
```
...
By default, Ravenox blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with .ravenox message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Ravenox replies in-thread.
