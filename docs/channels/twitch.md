...
read_when:
  - Setting up Twitch chat integration for Ravenox
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Ravenox connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ravenox plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `RAVENOX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: .ravenox", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use RAVENOX_TWITCH_ACCESS_TOKEN env var)
...
```bash
RAVENOX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
