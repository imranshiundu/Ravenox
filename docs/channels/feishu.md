...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Ravenox to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
ravenox plugins install ./extensions/feishu
```
...
If you just installed Ravenox, run the wizard:
...
2. Configuring app credentials in Ravenox
...
1. You already ran .ravenox channels add` for Feishu
2. The gateway is running (.ravenox gateway status`)
...
```bash
ravenox pairing approve feishu <CODE>
```
...
2. Run .ravenox logs --follow` and look for `chat_id`
...
2. Run .ravenox logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| .ravenox gateway status`  | Show gateway status           |
| .ravenox gateway install` | Install/start gateway service |
| .ravenox gateway stop`    | Stop gateway service          |
| .ravenox gateway restart` | Restart gateway service       |
| .ravenox logs --follow`   | Tail gateway logs             |
...
4. Check logs: .ravenox logs --follow`
...
5. Ensure the gateway is running: .ravenox gateway status`
6. Check logs: .ravenox logs --follow`
