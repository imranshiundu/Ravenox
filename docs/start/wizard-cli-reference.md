...
read_when:
  - You need detailed behavior for.ravenox onboard
...
This page is the full reference for .ravenox onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.ravenox.ravenox.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run .ravenox doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      .ravenox pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs .ravenox health`.
    - .ravenox status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
Typical fields in `~/.ravenox.ravenox.json`:
...
- Command reference: [.ravenox onboard`](/cli/onboard)
