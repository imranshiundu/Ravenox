---
summary: "Run Ravenox Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Ravenox running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Ravenox on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Ravenox Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Ravenox 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
```bash
gcloud projects create my.ravenox-project --name="Ravenox Gateway"
gcloud config set project my.ravenox-project
```
...
```bash
gcloud compute instances create.ravenox-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh.ravenox-gateway --zone=us-central1-a
```
...
## 6) Clone the Ravenox repository
...
```bash
OPENCLAW_IMAGE.ravenox:latest
OPENCLAW_GATEWAY_TOKEN=change-me-now
OPENCLAW_GATEWAY_BIND=lan
OPENCLAW_GATEWAY_PORT=18789

OPENCLAW_CONFIG_DIR=/home/$USER/.ravenox
OPENCLAW_WORKSPACE_DIR=/home/$USER/.ravenox/workspace
...
services:
 .ravenox-gateway:
    ...
    volumes:
      - ${OPENCLAW_CONFIG_DIR}:/home/node/.ravenox
      - ${OPENCLAW_WORKSPACE_DIR}:/home/node/.ravenox/workspace
    ports:
...
```bash
gcloud compute ssh.ravenox-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop.ravenox-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type.ravenox-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start.ravenox-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create.ravenox-deploy \
     --display-name="Ravenox Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my.ravenox-project \
     --member="serviceAccount.ravenox-deploy@my.ravenox-project.iam.gserviceaccount.com" \
