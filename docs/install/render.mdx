...
Deploy Ravenox on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com.ravenox.ravenox)
...
repository configures everything needed to run Ravenox:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: RAVENOX_STATE_DIR
        value: /data/.ravenox
      - key: RAVENOX_WORKSPACE_DIR
        value: /data/workspace
      - key: RAVENOX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com.ravenox`.
...
If you use the original Ravenox repository, Render will not auto-deploy your Ravenox. To update it, run a manual Blueprint sync from the dashboard.
...
For Ravenox, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Ravenox host.
