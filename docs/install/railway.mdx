...
Deploy Ravenox on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/ravenox-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>.ravenox` — Control UI

## What you get

- Hosted Ravenox Gateway + Control UI
...
- `RAVENOX_STATE_DIR=/data/.ravenox` (recommended)
- `RAVENOX_WORKSPACE_DIR=/data/workspace` (recommended)
- `RAVENOX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Ravenox state + workspace so you can migrate to another host without losing config or memory.
