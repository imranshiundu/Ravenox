...
```bash
ravenox tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with .ravenox tui --deliver`
...
- Check the Gateway logs: .ravenox logs --follow`.
- Confirm the agent can run: .ravenox status` and .ravenox models status`.
...
- No agents in picker: check .ravenox agents list` and your routing config.
