...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `.ravenox`)
...
If the page fails to load, start the Gateway first: .ravenox gateway`.
...
you revoke it with .ravenox devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.ravenox.ravenox.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Ravenox
...
Ravenox **blocks** Control UI connections without device identity.
