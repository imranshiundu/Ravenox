---
summary: "Use Venice AI privacy-focused models in Ravenox"
read_when:
  - You want privacy-focused inference in Ravenox
...
```bash
ravenox onboard --auth-choice venice-api-key
```
...
```bash
ravenox chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Ravenox shows all available Venice models. Pick based on your needs:
...
```bash
ravenox models set venice/claude-opus-45
ravenox models set venice/llama-3.3-70b
```

List all available models:

```bash
ravenox models list | grep venice
```

## Configure via .ravenox configure`

1. Run .ravenox configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Ravenox automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ravenox chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
ravenox chat --model venice/claude-opus-45

# Use uncensored model
ravenox chat --model venice/venice-uncensored

# Use vision model with image
ravenox chat --model venice/qwen3-vl-235b-a22b

# Use coding model
ravenox chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run .ravenox models list` to see currently available models. Some models may be temporarily offline.
