---
summary: "Use MiniMax M2.1 in Ravenox"
read_when:
  - You want MiniMax models in Ravenox
...
```bash
ravenox plugins enable minimax-portal-auth  # skip if already loaded.
ravenox gateway restart  # restart if gateway is already running
ravenox onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com.ravenox"@ravenox/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via .ravenox.json`:
...
- Use .ravenox models list` and .ravenox models set minimax/MiniMax-M2.1` to switch.
...
- Running .ravenox configure` and selecting **MiniMax M2.1**, or
