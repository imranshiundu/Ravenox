---
summary: "Use Synthetic's Anthropic-compatible API in Ravenox"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Ravenox registers it as the
...
Note: Ravenox's Anthropic client appends `/v1` to the base URL, so use
