...
ravenox cron list
ravenox cron run <job-id>
ravenox cron runs --id <job-id>
```
...
To reduce top-of-hour load spikes across many gateways, Ravenox applies a
...
If `delivery` is omitted for isolated jobs, Ravenox defaults to `announce`.
...
ravenox cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
ravenox cron edit <jobId> --agent ops
ravenox cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
ravenox cron run <jobId>
ravenox cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
ravenox cron edit <jobId> \
...
```bash
ravenox system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [.ravenox system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `RAVENOX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Ravenox applies exponential retry backoff for recurring jobs after consecutive errors:
