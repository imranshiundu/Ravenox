...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Ravenox.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Ravenox. See [Webhook Hooks](/automation/webhook) or use .ravenox webhooks` for Gmail helper commands.
...
- Extend Ravenox's behavior without modifying core code
...
```bash
ravenox hooks info session-memory
```

### Onboarding

During onboarding (.ravenox onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
Hook packs are standard npm packages that export one or more hooks via .ravenox.hooks` in
`package.json`. Install them with:

```bash
ravenox hooks install <path-or-spec>
```
...
The `metadata.ravenox` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Ravenox persists them.
...
ravenox hooks list --eligible

# Verbose output (show missing requirements)
ravenox hooks list --verbose

# JSON output
ravenox hooks list --json
```
...
# JSON output
ravenox hooks info session-memory --json
```
...
# Disable a hook
ravenox hooks disable command-logger
```
...
```bash
ravenox hooks enable bootstrap-extra-files
```
