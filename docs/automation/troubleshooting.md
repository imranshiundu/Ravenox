...
ravenox system heartbeat last
ravenox logs --follow
ravenox config get agents.defaults.heartbeat
ravenox channels status --probe
```
...
```bash
ravenox config get agents.defaults.heartbeat.activeHours
ravenox config get agents.defaults.heartbeat.activeHours.timezone
ravenox config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
ravenox cron list
ravenox logs --follow
```
