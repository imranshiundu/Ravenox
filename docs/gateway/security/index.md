...
## Quick check: .ravenox security audit`
...
Ravenox is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Ravenox also attempts a best-effort live Gateway probe.
...
| -------------------------------------------- | ------------- | -------------------------------------------------------- | ------------------------------------------------ | -------- |
| `fs.state_dir.perms_world_writable`          | critical      | Other users/processes can modify full Ravenox state     | filesystem perms on `~/.ravenox`                | yes      |
| `fs.config.perms_writable`                   | critical      | Others can change auth/tool policy/config                | filesystem perms on `~/.ravenox.ravenox.json`  | yes      |
...
.ravenox security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${RAVENOX_GATEWAY_PASSWORD}
```
...
Ravenox can refresh the skills list mid-session:
...
ravenox pairing approve <channel> <code>
```
...
By default, Ravenox routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
- “Paste the full contents of ~/.ravenox or your logs.”

## Unsafe external content bypass flags

Ravenox includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.ravenox.ravenox.json`: `600` (user read/write only)
- `~/.ravenox`: `700` (user only)

.ravenox doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `RAVENOX_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/_.ravenox__/canvas/` and `/_.ravenox__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`.ravenox-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `RAVENOX_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: .ravenox doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `RAVENOX_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `RAVENOX_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Ravenox
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Ravenox verifies the identity by resolving the
...
- Ravenox will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- When sharing diagnostics, prefer .ravenox status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": [".ravenox", "@mybot"] }
      }
    ]
  }
}
...
- Prefer a dedicated profile for the agent (the default .ravenox` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Ravenox clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Ravenox limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your .ravenox gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `RAVENOX_GATEWAY_PASSWORD`) and restart.
...
- Timestamp, gateway host OS + Ravenox version
...
Found a vulnerability in Ravenox? Please report responsibly:

1. Email: [security.ravenox.ai](mailto:security.ravenox.ai)
