...
- If you install plugins from npm (.ravenox plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.ravenox/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
Assume anything under `~/.ravenox/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:
