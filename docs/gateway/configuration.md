...
read_when:
  - Setting up Ravenox for the first time
...
Ravenox reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.ravenox.ravenox.json`.

If the file is missing, Ravenox uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with .ravenox onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
   .ravenox onboard       # full setup wizard
   .ravenox configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.ravenox.ravenox.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Ravenox only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (.ravenox doctor`, .ravenox logs`, .ravenox health`, .ravenox status`)
- Run .ravenox doctor` to see exact issues
- Run .ravenox doctor --fix` (or `--yes`) to apply repairs
...
The Gateway watches `~/.ravenox.ravenox.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or .ravenox config set` for single keys.
    </Warning>
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Ravenox runs your login shell and imports only the missing keys:
...
Env var equivalent: `RAVENOX_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${RAVENOX_GATEWAY_TOKEN}" } },
