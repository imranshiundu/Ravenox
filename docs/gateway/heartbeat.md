...
- During heartbeat runs, Ravenox treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from .ravenox sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Ravenox skips the heartbeat run entirely (no model call).
...
```bash
ravenox system event --text "Check for urgent follow-ups" --mode now
```
