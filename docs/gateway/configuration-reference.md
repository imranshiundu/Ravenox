...
description: "Complete field-by-field reference for ~/.ravenox.ravenox.json"
---

# Configuration Reference

Every field available in `~/.ravenox.ravenox.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Ravenox uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by .ravenox doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: [.ravenox-dm"] },
      guilds: {
        "123456789012345678": {
...
Ravenox spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", ".ravenox"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes .ravenox.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Ravenox auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Ravenox assumes `anthropic` (deprecated).
    ...
    list: [
      {
        ...
        groupChat: { mentionPatterns: [".ravenox"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group.ravenox`   | All built-in tools (excludes provider plugins)                                           |
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: .ravenox onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: .ravenox onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or .ravenox onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: .ravenox onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: .ravenox onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: .ravenox onboard --auth-choice minimax-api`.
...
Convenience flags: `--dev` (uses `~/.ravenox-dev` + port `19001`), `--profile <name>` (uses `~/.ravenox-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x.ravenox-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/_.ravenox__/canvas/`
  - `http://<gateway-host>:<gateway.port>/_.ravenox__/a2ui/`
...
- Also serves A2UI at `/_.ravenox__/a2ui/`.
...
- Default log file: `/tmp.ravenox.ravenox-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; .ravenox doctor --fix` can strip unknown keys).
