---
read_when:
  - 你想在 Ravenox 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Ravenox 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
ravenox plugins enable minimax-portal-auth  # 如果已加载则跳过
ravenox gateway restart  # 如果 Gateway 网关已在运行则重启
...
- 使用 .ravenox models list` 和 .ravenox models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 .ravenox configure` 并选择 **MiniMax M2.1**，或
