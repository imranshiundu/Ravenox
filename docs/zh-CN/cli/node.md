...
summary: ".ravenox node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
```bash
ravenox node install --host <gateway-host> --port 18789
```
...
ravenox node uninstall
```

使用 .ravenox node run` 运行前台节点主机（无服务）。
...
- .ravenox approvals --node <id|name|ip>`（从 Gateway 网关编辑）
