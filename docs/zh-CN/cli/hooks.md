...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：.ravenox hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/"@ravenox/config.json`）来启用它。

**注意：** 由插件管理的钩子在 .ravenox hooks list` 中显示 `plugin:<id>`，
...
```bash
# 本地目录
ravenox hooks install ./my-hook-pack
...
```bash
ravenox hooks update <id>
ravenox hooks update --all
```
...
**输出：** `~/"@ravenox/workspace/memory/YYYY-MM-DD-slug.md`
