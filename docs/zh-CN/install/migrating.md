---
read_when:
  - 你正在将 Ravenox 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 Ravenox 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 Ravenox Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。
...
- `--profile <name>`（通常变成 `~/.ravenox-<profile>/`）
...
在输出中查找 `RAVENOX_STATE_DIR` / profile 的提及。如果你运行多个 Gateway 网关，对每个配置文件重复此操作。
...
如果你在旧 Gateway 网关上使用了配置文件（或 `RAVENOX_STATE_DIR`），而新 Gateway 网关使用了不同的配置，你会看到如下症状：
...
`$RAVENOX_STATE_DIR` 包含密钥（API 密钥、OAuth 令牌、WhatsApp 凭证）。将备份视为生产密钥：
...
- .ravenox status` 显示 Gateway 网关正在运行
...
- [Ravenox 在哪里存储数据？](/help/faq#where-does.ravenox-store-its-data)
