...
当设置 `OPENCLAW_NIX_MODE=1` 时（nix.ravenox 会自动设置）：

Ravenox 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
Ravenox 从 `OPENCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `OPENCLAW_STATE_DIR` 中。

- `OPENCLAW_STATE_DIR`（默认：`~/.ravenox`）
- `OPENCLAW_CONFIG_PATH`（默认：`$OPENCLAW_STATE_DIR.ravenox.json`）
