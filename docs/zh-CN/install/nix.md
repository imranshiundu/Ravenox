...
当设置 `RAVENOX_NIX_MODE=1` 时（nix.ravenox 会自动设置）：
...
Ravenox 从 `RAVENOX_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `RAVENOX_STATE_DIR` 中。

- `RAVENOX_STATE_DIR`（默认：`~/.ravenox`）
- `RAVENOX_CONFIG_PATH`（默认：`$RAVENOX_STATE_DIR.ravenox.json`）
