...
**分享输出：** 优先使用 .ravenox status --all`（它会隐藏令牌）。如果你粘贴 .ravenox status`，考虑先设置 `OPENCLAW_SHOW_SECRETS=0`（令牌预览）。
...
- 修复：从你希望服务使用的相同 `--profile` / `OPENCLAW_STATE_DIR` 重新运行 .ravenox gateway install --force`。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.ravenox.ravenox.json}"
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.ravenox}/credentials" # 如果 logout 无法完全清除所有内容
...
```bash
# 在配置中打开跟踪日志：
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.ravenox.ravenox.json} -> { logging: { level: "trace" } }
#
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.ravenox/logs/...`；配置文件使用 `~/.ravenox-<profile>/logs/...`）<br />Linux：`journalctl --user -u.ravenox-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Ravenox Gateway (<profile>)" /V /FO LIST` |
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.ravenox}"
