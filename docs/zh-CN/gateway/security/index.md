...
- 收紧本地权限（`~/.ravenox` → `700`，配置文件 → `600`，以及常见状态文件如 `credentials/*.json`、`agents/*/agent/auth-profiles.json` 和 `agents/*/sessions/sessions.json`）。

在你的机器上运行具有 shell 访问权限的 AI 智能体是……_有风险的_。以下是如何避免被攻击的方法。

Ravenox 既是产品也是实验：你正在将前沿模型的行为连接到真实的消息平台和真实的工具。**不存在"完美安全"的设置。** 目标是有意识地考虑：
...
如果运行 `--deep`，Ravenox 还会尝试尽力进行实时 Gateway 网关探测。
...
.ravenox security audit` 会在启用此设置时发出警告。
...
默认情况下，Ravenox 将**所有私信路由到主会话**，以便你的助手在设备和渠道之间保持连续性。如果**多人**可以给机器人发私信（开放私信或多人白名单），请考虑隔离私信会话：
...
4. **重新运行审计**
   - .ravenox security audit --deep` 并确认报告是干净的。
...
- `~/.ravenox.ravenox.json`：`600`（仅用户读/写）
- `~/.ravenox`：`700`（仅用户）

.ravenox doctor` 可以警告并提供收紧这些权限的选项。
...
- 默认：`18789`
- 配置/标志/环境变量：`gateway.port`、`--port`、`RAVENOX_GATEWAY_PORT`
...
Gateway 网关通过 mDNS（端口 5353 上的 `.ravenox-gw._tcp`）广播其存在以用于本地设备发现。在完整模式下，这包括可能暴露运营详情的 TXT 记录：
...
4. **环境变量**（替代方案）：设置 `RAVENOX_DISABLE_BONJOUR=1` 以在不更改配置的情况下禁用 mDNS。
...
Doctor 可以为你生成一个：.ravenox doctor --generate-gateway-token`。
...
- `gateway.auth.mode: "password"`：密码认证（优先通过环境变量设置：`RAVENOX_GATEWAY_PASSWORD`）。

轮换清单（令牌/密码）：

1. 生成/设置一个新的秘密（`gateway.auth.token` 或 `RAVENOX_GATEWAY_PASSWORD`）。
...
当 `gateway.auth.allowTailscale` 为 `true`（Serve 的默认值）时，Ravenox 接受 Tailscale Serve 身份头（`tailscale-user-login`）作为认证。Ravenox 通过本地 Tailscale 守护进程（`tailscale whois`）解析 `x-forwarded-for` 地址并将其与头匹配来验证身份。这仅对命中回环并包含 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host`（由 Tailscale 注入）的请求触发。
...
- Ravenox 将信任来自这些 IP 的 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP 以进行本地配对检查和 HTTP 认证/本地检查。
...
- 共享诊断信息时，优先使用 .ravenox status --all`（可粘贴，秘密已脱敏）而不是原始日志。
...
- 优先为智能体使用专用配置文件（默认的 .ravenox` 配置文件）。
...
- Chrome 扩展中继的 CDP 端点是认证门控的；只有 Ravenox 客户端可以连接。
...
1. **停止它：** 停止 macOS 应用（如果它监督 Gateway 网关）或终止你的 .ravenox gateway` 进程。
...
1. 轮换 Gateway 网关认证（`gateway.auth.token` / `RAVENOX_GATEWAY_PASSWORD`）并重启。
...
- 时间戳、Gateway 网关主机操作系统 + Ravenox 版本
- 会话记录 + 短日志尾部（脱敏后）
- 攻击者发送了什么 + 智能体做了什么
...
在 Ravenox 中发现漏洞？请负责任地报告：

1. 电子邮件：security.ravenox.ai
2. 在修复之前不要公开发布
3. 我们会感谢你（除非你希望匿名）
