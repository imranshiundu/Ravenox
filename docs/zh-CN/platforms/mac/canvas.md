...
- `~/Library/Application Support/Ravenox/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- .ravenox-canvas://<session>/<path>`

示例：

- .ravenox-canvas://main/` → `<canvasRoot>/main/index.html`
- .ravenox-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- .ravenox-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
ravenox nodes canvas present --node <id>
ravenox nodes canvas navigate --node <id> --url "/"
ravenox nodes canvas eval --node <id> --js "document.title"
ravenox nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/_.ravenox__/a2ui/
```
...
ravenox nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
ravenox nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = .ravenox://agent?message=Review%20this%20design";
```
