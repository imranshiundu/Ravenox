...
你可以在 `~/.ravenox.ravenox.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to.ravenox.log"
  }
}
```
...
```bash
ravenox channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.ravenox.ravenox.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp.ravenox.ravenox-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Ravenox 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 .ravenox plugins enable diagnostics-otel` 启用插件。
...
- .ravenox.tokens`（计数器，属性：.ravenox.token`、.ravenox.channel`、.ravenox.provider`、.ravenox.model`）
- .ravenox.cost.usd`（计数器，属性：.ravenox.channel`、.ravenox.provider`、.ravenox.model`）
- .ravenox.run.duration_ms`（直方图，属性：.ravenox.channel`、.ravenox.provider`、.ravenox.model`）
- .ravenox.context.tokens`（直方图，属性：.ravenox.context`、.ravenox.channel`、.ravenox.provider`、.ravenox.model`）

消息流：

- .ravenox.webhook.received`（计数器，属性：.ravenox.channel`、.ravenox.webhook`）
- .ravenox.webhook.error`（计数器，属性：.ravenox.channel`、.ravenox.webhook`）
- .ravenox.webhook.duration_ms`（直方图，属性：.ravenox.channel`、.ravenox.webhook`）
- .ravenox.message.queued`（计数器，属性：.ravenox.channel`、.ravenox.source`）
- .ravenox.message.processed`（计数器，属性：.ravenox.channel`、.ravenox.outcome`）
- .ravenox.message.duration_ms`（直方图，属性：.ravenox.channel`、.ravenox.outcome`）

队列 + 会话：

- .ravenox.queue.lane.enqueue`（计数器，属性：.ravenox.lane`）
- .ravenox.queue.lane.dequeue`（计数器，属性：.ravenox.lane`）
- .ravenox.queue.depth`（直方图，属性：.ravenox.lane` 或 .ravenox.channel=heartbeat`）
- .ravenox.queue.wait_ms`（直方图，属性：.ravenox.lane`）
- .ravenox.session.state`（计数器，属性：.ravenox.state`、.ravenox.reason`）
- .ravenox.session.stuck`（计数器，属性：.ravenox.state`）
- .ravenox.session.stuck_age_ms`（直方图，属性：.ravenox.state`）
- .ravenox.run.attempt`（计数器，属性：.ravenox.attempt`）

### 导出的 span（名称 + 关键属性）

- .ravenox.model.usage`
  - .ravenox.channel`、.ravenox.provider`、.ravenox.model`
  - .ravenox.sessionKey`、.ravenox.sessionId`
  - .ravenox.tokens.*`（input/output/cache_read/cache_write/total）
- .ravenox.webhook.processed`
  - .ravenox.channel`、.ravenox.webhook`、.ravenox.chatId`
- .ravenox.webhook.error`
  - .ravenox.channel`、.ravenox.webhook`、.ravenox.chatId`、.ravenox.error`
- .ravenox.message.processed`
  - .ravenox.channel`、.ravenox.outcome`、.ravenox.chatId`、.ravenox.messageId`、.ravenox.sessionKey`、.ravenox.sessionId`、.ravenox.reason`
- .ravenox.session.stuck`
  - .ravenox.state`、.ravenox.ageMs`、.ravenox.queueDepth`、.ravenox.sessionKey`、.ravenox.sessionId`
...
- **Gateway 网关无法访问？** 先运行 .ravenox doctor`。
