...
read_when:
  - 新手引导新助手实例时
  - 审查安全/权限影响时
summary: 将 Ravenox 作为个人助手运行的端到端指南，包含安全注意事项
title: 个人助手设置
x-i18n:
  ...
  source_path: start.ravenox.md
  workflow: 15
---

# 使用 Ravenox 构建个人助手

Ravenox 是 **Pi** 智能体的 WhatsApp + Telegram + Discord + iMessage Gateway 网关。插件可添加 Mattermost。本指南是"个人助手"设置：一个专用的 WhatsApp 号码，表现得像你的常驻智能体。
...
- Node **22+**
- Ravenox 在 PATH 中可用（推荐：全局安装）
- 助手的第二个手机号码（SIM/eSIM/预付费）

```bash
npm install -g.ravenox@latest
# 或：pnpm add -g.ravenox@latest
```
...
如果你将个人 WhatsApp 关联到 Ravenox，发给你的每条消息都会变成"智能体输入"。这通常不是你想要的。
...
3. 在 `~/.ravenox.ravenox.json` 中放置最小配置：
...
新手引导完成后，我们会自动打开带有 Gateway 网关令牌的仪表板并打印带令牌的链接。稍后重新打开：.ravenox dashboard`。
...
默认情况下，Ravenox 每 30 分钟运行一次心跳，提示词为：
`Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.`
设置 `agents.defaults.heartbeat.every: "0m"` 来禁用。

- 如果 `HEARTBEAT.md` 存在但实际上是空的（只有空行和 markdown 标题如 `# Heading`），Ravenox 会跳过心跳运行以节省 API 调用。
- 如果文件不存在，心跳仍然运行，模型决定做什么。
- 如果智能体回复 `HEARTBEAT_OK`（可选带有短填充；参见 `agents.defaults.heartbeat.ackMaxChars`），Ravenox 会为该心跳抑制出站投递。
...
Ravenox 会提取这些并将它们作为媒体与文本一起发送。

## 运维检查清单

```bash
ravenox status          # 本地状态（凭证、会话、排队事件）
ravenox status --all    # 完整诊断（只读，可粘贴）
ravenox status --deep   # 添加 Gateway 网关健康探测（Telegram + Discord）
...
- macOS 菜单栏配套应用：[Ravenox macOS 应用](/platforms/macos)
