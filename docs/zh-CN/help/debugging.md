...
当你需要切换不常用的设置而不编辑 .ravenox.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.ravenox-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.ravenox/logs/raw-stream.jsonl
```
