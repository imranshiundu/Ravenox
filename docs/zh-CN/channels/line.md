...
LINE 通过 LINE Messaging API 连接到 Ravenox。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
ravenox plugins install ./extensions/line
```
...
```bash
ravenox pairing list line
ravenox pairing approve line <CODE>
```
