...
read_when:
  - Designing workspace memory (~/.ravenox/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Ravenox integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.ravenox/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.ravenox/workspace` as canonical human-readable memory.
...
```
~/.ravenox/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Ravenox**, but keep a separable core library.

### Why integrate into Ravenox?

- Ravenox already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - .ravenox memory recall "…" --k 25 --since 30d`
  - .ravenox memory reflect --since 7d`
...
Pragmatic take for `~/.ravenox/workspace`:
