...
This document explains how Ravenox manages sessions end-to-end:
...
Ravenox is designed around a single **Gateway process** that owns session state.
...
Ravenox persists sessions in two layers:
...
Ravenox intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Ravenox consumes the events, but Pi decides when to compact).
...
Ravenox also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Ravenox bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Ravenox leaves it alone.
...
- .ravenox status` (CLI)
- .ravenox sessions` / `sessions --json`
...
Ravenox supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Ravenox strips/suppresses this in the delivery layer.

As of `2026.1.10`, Ravenox also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Ravenox uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Ravenox’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from .ravenox status`.
