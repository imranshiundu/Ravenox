...
Ravenox features that can generate provider usage or paid API calls.
...
- .ravenox status --usage` and .ravenox channels list` show provider **usage windows**
...
Ravenox can pick up credentials from:
...
- .ravenox status --usage`
- .ravenox models status --json`
...
.ravenox models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
