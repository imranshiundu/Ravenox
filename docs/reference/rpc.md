...
Ravenox integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Ravenox owns lifecycle when `channels.signal.autoStart=true`.
...
- Ravenox spawns `imsg rpc` as a child process (legacy iMessage integration).
