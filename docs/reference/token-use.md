---
summary: "How Ravenox builds prompt context and reports token usage + costs"
read_when:
...
Ravenox tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Ravenox assembles its own system prompt on every run. It includes:
...
For images, Ravenox downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Ravenox session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** .ravenox status --usage` and .ravenox channels list` show
...
`cacheWrite`. If pricing is missing, Ravenox shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Ravenox can
...
Anthropic's 1M context window is currently beta-gated. Ravenox can inject the
