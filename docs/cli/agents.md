---
summary: "CLI reference for .ravenox agents` (list/add/delete/set identity)"
read_when:
  - You want multiple isolated agents (workspaces + routing + auth)
title: "agents"
---

# .ravenox agents`

Manage isolated agents (workspaces + auth + routing).

Related:

- Multi-agent routing: [Multi-Agent Routing](/concepts/multi-agent)
- Agent workspace: [Agent workspace](/concepts/agent-workspace)

## Examples

```bash
ravenox agents list
ravenox agents add work --workspace ~/.ravenox/workspace-work
ravenox agents set-identity --workspace ~/.ravenox/workspace --from-identity
ravenox agents set-identity --agent main --avatar avatars.ravenox.png
ravenox agents delete work
```

## Identity files

Each agent workspace can include an `IDENTITY.md` at the workspace root:

- Example path: `~/.ravenox/workspace/IDENTITY.md`
- `set-identity --from-identity` reads from the workspace root (or an explicit `--identity-file`)

Avatar paths resolve relative to the workspace root.

## Set identity

`set-identity` writes fields into `agents.list[].identity`:

- `name`
- `theme`
- `emoji`
- `avatar` (workspace-relative path, http(s) URL, or data URI)

Load from `IDENTITY.md`:

```bash
ravenox agents set-identity --workspace ~/.ravenox/workspace --from-identity
```

Override fields explicitly:

```bash
ravenox agents set-identity --agent main --name "Ravenox" --emoji "🦞" --avatar avatars.ravenox.png
```

Config sample:

```json5
{
  agents: {
    list: [
      {
        id: "main",
        identity: {
          name: "Ravenox",
          theme: "space lobster",
          emoji: "🦞",
          avatar: "avatars.ravenox.png",
        },
      },
    ],
  },
}
```
