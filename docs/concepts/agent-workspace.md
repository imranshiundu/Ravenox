...
.ravenox onboard`, .ravenox configure`, or .ravenox setup` will create the
...
Older installs may have created `~.ravenox`. Keeping multiple workspace
directories around can cause confusing auth or state drift, because only one
workspace is active at a time.

**Recommendation:** keep a single active workspace. If you no longer use the
extra folders, archive or move them to Trash (for example `trash ~.ravenox`).
If you intentionally keep multiple workspaces, make sure
`agents.defaults.workspace` points to the active one.

.ravenox doctor` warns when it detects extra workspace directories.

## Workspace file map (what each file means)

These are the standard files Ravenox expects inside the workspace:
...
If any bootstrap file is missing, Ravenox injects a "missing file" marker into
the session and continues. Large bootstrap files are truncated when injected;
adjust limits with `agents.defaults.bootstrapMaxChars` (default: 20000) and
`agents.defaults.bootstrapTotalMaxChars` (default: 150000).
.ravenox setup` can recreate missing defaults without overwriting existing
files.
