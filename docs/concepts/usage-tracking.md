...
- `/usage cost` in chats: local cost summary aggregated from Ravenox session logs.
- CLI: .ravenox status --usage` prints a full per-provider breakdown.
- CLI: .ravenox channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
