...
read_when:
  - You want to place an outbound voice call from Ravenox
...
Voice calls for Ravenox via a plugin. Supports outbound notifications and
...
- Use .ravenox voicecall ...` or the `voice_call` tool
...
```bash
ravenox plugins install ./extensions/voice-call
