---
summary: "Ravenox on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Ravenox on a Raspberry Pi
  - Running Ravenox on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Ravenox on Raspberry Pi

## Goal

Run a persistent, always-on Ravenox Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Ravenox features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled.ravenox

# Enable if not
sudo systemctl enable.ravenox

# Start on boot
sudo systemctl start.ravenox
```
...
```bash
# Check logs
journalctl -u.ravenox --no-pager -n 100

# Common fix: rebuild
cd ~.ravenox  # if using hackable install
npm run build
sudo systemctl restart.ravenox
```
