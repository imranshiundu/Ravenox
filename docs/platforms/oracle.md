---
summary: "Ravenox on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Ravenox on Oracle Cloud
  - Looking for low-cost VPS hosting for Ravenox
  - Want 24/7 Ravenox on a small server
title: "Oracle Cloud"
---

# Ravenox on Oracle Cloud (OCI)

## Goal

Run a persistent Ravenox Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Ravenox (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname.ravenox
...
sudo tailscale up --ssh --hostname.ravenox
```

This enables Tailscale SSH, so you can connect via `ssh.ravenox` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu.ravenox` (or use the Tailscale IP).
...
ravenox config set gateway.bind loopback

# Require auth for the Gateway + Control UI
ravenox config set gateway.auth.mode token
ravenox doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
ravenox config set gateway.tailscale.mode serve
ravenox config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart.ravenox-gateway
```
...
# Check daemon status
systemctl --user status.ravenox-gateway
...
```
https:/.ravenox.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run .ravenox security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.ravenox`
- **Security audit:** .ravenox security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu.ravenox
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname.ravenox --reset
```
...
journalctl --user -u.ravenox-gateway -n 50
```
