...
2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.ravenox`) from the old machine.
...
Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.ravenox`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$OPENCLAW_STATE_DIR.ravenox.json`                             | Main config (JSON5)                                          |
...
Ravenox reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.ravenox.ravenox.json`):
...
- a global fallback `.env` from `~/.ravenox/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.ravenox-<profile>`).
...
- macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.ravenox/logs/...`; profiles use `~/.ravenox-<profile>/logs/...`)
