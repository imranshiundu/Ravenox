---
summary: "Ravenox plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Ravenox with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Ravenox yet (or you want to keep optional features out of your main
install).
...
Ravenox plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
or .ravenox plugins enable <id>`. Installed plugins are enabled by default,
but can be disabled the same way.

Each plugin must include a .ravenox.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with .ravenox.extensions`:
...
Security note: .ravenox plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via .ravenox.channel` and
install hints via .ravenox.install`. This keeps the core catalog data-free.
...
If a plugin exports `id`, Ravenox uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  .ravenox.plugin.json` (`configSchema`).
...
Ravenox augments `uiHints` at runtime based on discovered plugins:
...
ravenox plugins disable <id>
ravenox plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.

Plugins may also register their own top‑level commands (example: .ravenox voicecall`).
...
- Plugin-managed hooks show up in .ravenox hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via .ravenox hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside Ravenox (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- .ravenox models auth login --provider <id> [--method <id>]`
...
- `config`: The current Ravenox config
...
- CLI: .ravenox voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate .ravenox.extensions` points at the built entrypoint (`dist/index.js`).
