...
- **gateway host** → .ravenox` process on the gateway machine
...
`~/"@ravenox/exec-approvals.json` directly.

CLI: .ravenox approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
