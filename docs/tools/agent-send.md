---
summary: "Direct .ravenox agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# .ravenox agent` (direct agent runs)

.ravenox agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match .ravenox message --target`).
...
```bash
ravenox agent --to +15555550123 --message "status update"
...
ravenox agent --to +15555550123 --message "Summon reply" --deliver
