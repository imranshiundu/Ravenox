---
summary: "Agent tool surface for Ravenox (browser, canvas, nodes, message, cron) replacing legacy .ravenox-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing .ravenox-*` skills
title: "Tools"
---

# Tools (Ravenox)

Ravenox exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old .ravenox-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in .ravenox.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Ravenox logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group.ravenox`: all built-in Ravenox tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (.ravenox node run`).
...
Ravenox tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: .ravenox configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Ravenox-managed browser.
...
- Quick smoke: .ravenox nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; .ravenox gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Ravenox clamps `tools.sessions.visibility` to `tree`.
