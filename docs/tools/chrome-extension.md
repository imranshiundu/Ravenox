---
summary: "Chrome extension: let Ravenox drive your existing Chrome tab"
read_when:
...
The Ravenox Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate.ravenox-managed Chrome profile.
...
Ravenox then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Ravenox release (npm package) as static files. There is no separate “build” step.

After upgrading Ravenox:

- Re-run .ravenox browser extension install` to refresh the installed files under your Ravenox state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Ravenox ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `RAVENOX_GATEWAY_TOKEN`)
...
- Open the tab you want Ravenox to control.
...
- `ON`: attached; Ravenox can drive that tab.
...
Debugging: .ravenox sandbox explain`
...
.ravenox browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run .ravenox browser extension install` first to copy the extension to a stable location under your Ravenox state directory.
...
- **This is not isolated** like the dedicated.ravenox-managed profile.
