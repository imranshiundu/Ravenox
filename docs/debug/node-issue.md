...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Ravenox
...
Running Ravenox via Node with `tsx` fails at startup with:

```
.ravenox] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), .ravenox status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch.ravenox.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node.ravenox.mjs status` works on Node 25.
