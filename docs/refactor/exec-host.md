...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/"@ravenox/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/"@ravenox/node.json` (node id + pairing token).
...
Path: `~/"@ravenox/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/"@ravenox/exec-approvals.sock",
...
- Unix socket at `~/"@ravenox/exec-approvals.sock` (0600).
