...
export async function handleWhatsAppStart(host: RavenoxApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: RavenoxApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: RavenoxApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: RavenoxApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: RavenoxApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: RavenoxApp): string {
...
function resolveGatewayHttpAuthHeader(host: RavenoxApp): string | null {
...
function buildGatewayHttpHeaders(host: RavenoxApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: RavenoxApp) {
...
export function handleNostrProfileToggleAdvanced(host: RavenoxApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: RavenoxApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: RavenoxApp) {
  const state = host.nostrProfileFormState;
