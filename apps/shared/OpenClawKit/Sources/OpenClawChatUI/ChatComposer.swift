...
@MainActor
struct RavenoxChatComposer: View {
    @Bindable var viewModel: RavenoxChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(RavenoxChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(RavenoxChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \RavenoxPendingAttachment.id)
                { (att: RavenoxPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            RavenoxPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(RavenoxChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(RavenoxChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(RavenoxChatTheme.subtleCard)
        .clipShape(Capsule())
    }
