...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: RavenoxProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: RavenoxProtocol.AnyCodable] = [
            "presence": .init([RavenoxProtocol.AnyCodable(entry)]),
        ]
        let payload = RavenoxProtocol.AnyCodable(payloadMap)
