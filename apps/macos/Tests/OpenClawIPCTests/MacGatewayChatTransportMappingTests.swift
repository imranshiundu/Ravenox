...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: RavenoxProtocol.AnyCodable(["ok": RavenoxProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: RavenoxProtocol.AnyCodable(["ok": RavenoxProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = RavenoxProtocol.AnyCodable([
            "runId": RavenoxProtocol.AnyCodable("run-1"),
            "sessionKey": RavenoxProtocol.AnyCodable("main"),
            "state": RavenoxProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: RavenoxProtocol.AnyCodable(["a": RavenoxProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
