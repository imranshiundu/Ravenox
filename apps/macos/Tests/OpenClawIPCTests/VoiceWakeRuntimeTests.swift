...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = [.ravenox"]
        let text = "hey.ravenox"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey.ravenox do thing"
        ...
        let config = WakeWordGateConfig(triggers: [.ravenox"], minPostTriggerGap: 0.3)
