...
enum DebugActions {
    private static let verboseDefaultsKey = .ravenox.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = .ravenox.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = RavenoxPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Ravenox", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = RavenoxPaths.configURL
        guard
