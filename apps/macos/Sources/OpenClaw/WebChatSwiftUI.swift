...
private let webChatSwiftLogger = Logger(subsystem: "ai.ravenox", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: RavenoxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> RavenoxChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(RavenoxChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> RavenoxChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                RavenoxGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<RavenoxChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any RavenoxChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = RavenoxChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: RavenoxChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<RavenoxChatView>) -> NSViewController
    {
