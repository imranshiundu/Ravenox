...
enum RavenoxLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = RavenoxOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = RavenoxFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        RavenoxLogging.bootstrapIfNeeded()
        let label = RavenoxLogging.makeLabel(subsystem: subsystem, category: category)
...
struct RavenoxOSLogHandler: LogHandler {
...
struct RavenoxFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = RavenoxLogging.parseLabel(self.label)
