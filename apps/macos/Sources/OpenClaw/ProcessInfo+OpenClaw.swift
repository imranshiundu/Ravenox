...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.ravenox.mac`) even if the shipped
    ...
    {
        if environment["RAVENOX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: .ravenox.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: .ravenox.nixMode") { return true }
