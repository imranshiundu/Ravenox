...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Ravenox token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: RavenoxOAuthStore.AnthropicOAuthStatus = RavenoxOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.ravenox", category: "anthropic-oauth")
...
enum RavenoxOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let.ravenoxOAuthDirEnv = "RAVENOX_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Ravenox OAuth token file not found"
            case .unreadableFile: "Ravenox OAuth token file not readable"
            case .invalidJSON: "Ravenox OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Ravenox OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Ravenox OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.ravenoxOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".ravenox", isDirectory: true)
