...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.ravenox.ravenox.json. Prefer RAVENOX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.ravenox.ravenox.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.ravenox.ravenox.json. Restart the gateway to apply."
        }
