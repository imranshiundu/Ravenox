...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.ravenox", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Ravenox run already has an SSH listener on the expected port (common after restarts),
