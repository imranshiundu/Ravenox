...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["RAVENOX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp.ravenox")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent(.ravenox-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent(.ravenox-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp.ravenox/ (rolling or stdout), or nil if none exist.
