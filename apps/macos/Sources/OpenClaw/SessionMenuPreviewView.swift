...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.ravenox", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> RavenoxSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: RavenoxSessionPreviewEntry,
    ...
    private static func cache(payload: RavenoxSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: RavenoxChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [RavenoxKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [RavenoxKit.AnyCodable]) -> [RavenoxChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(RavenoxChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: RavenoxChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: RavenoxChatMessage) -> Bool {
    ...
    private static func toolNames(for message: RavenoxChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: RavenoxChatMessage) -> String? {
