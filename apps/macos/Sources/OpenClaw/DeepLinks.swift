...
private let deepLinkLogger = Logger(subsystem: "ai.ravenox", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Ravenox is paused", message: "Unpause Ravenox to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Ravenox agent?", message: body) else { return }
        }
