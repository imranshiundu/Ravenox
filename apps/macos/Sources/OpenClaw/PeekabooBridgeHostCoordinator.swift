...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.ravenox", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: RavenoxPeekabooBridgeServices?
    private static var.ravenoxSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Ravenox", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = RavenoxPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.ravenoxSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.ravenoxSocketPath, privacy: .public)")
    }
...
@MainActor
private final class RavenoxPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.ravenox.peekaboo")
