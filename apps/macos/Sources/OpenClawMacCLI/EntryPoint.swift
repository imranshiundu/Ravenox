...
struct RavenoxMacCLI {
    static func main() async {
        ...
        default:
            fputs(.ravenox-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
   ...
   .ravenox-mac
    ...
    Usage:
     .ravenox-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
     .ravenox-mac discover [--timeout <ms>] [--json] [--include-local]
     .ravenox-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
     .ravenox-mac connect
     .ravenox-mac connect --url ws://127.0.0.1:18789 --json
     .ravenox-mac discover --timeout 3000 --json
     .ravenox-mac wizard --mode local
    """)
}
