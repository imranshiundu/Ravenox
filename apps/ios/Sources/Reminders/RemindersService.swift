...
final class RemindersService: RemindersServicing {
    func list(params: RavenoxRemindersListParams) async throws -> RavenoxRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[RavenoxReminderPayload], Error>) in
        ...
        return RavenoxRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: RavenoxRemindersAddParams) async throws -> RavenoxRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = RavenoxReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return RavenoxRemindersAddPayload(reminder: payload)
    }
