...
struct IOSGatewayChatTransport: RavenoxChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.ravenox", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> RavenoxChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(RavenoxChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> RavenoxChatHistoryPayload {
        ...
        return try JSONDecoder().decode(RavenoxChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [RavenoxChatAttachmentPayload]) async throws -> RavenoxChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [RavenoxChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(RavenoxChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(RavenoxGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RavenoxGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RavenoxChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RavenoxAgentEventPayload.self)
                        {
