...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> RavenoxNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> RavenoxNetworkStatusPayload {
        let status: RavenoxNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> RavenoxNetworkStatusPayload {
        RavenoxNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
