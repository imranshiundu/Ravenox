...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.ravenox.gateway"
    private static let nodeService = "ai.ravenox.node"
    private static let talkService = "ai.ravenox.talk"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.ravenox.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.ravenox.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent(.ravenox-gateway.log")
    }
