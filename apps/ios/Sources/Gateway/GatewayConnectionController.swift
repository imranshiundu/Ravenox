...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [RavenoxCapability.canvas.rawValue, RavenoxCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(RavenoxCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(RavenoxCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = RavenoxLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(RavenoxCapability.location.rawValue) }

        caps.append(RavenoxCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(RavenoxCapability.watch.rawValue)
        }
        caps.append(RavenoxCapability.photos.rawValue)
        caps.append(RavenoxCapability.contacts.rawValue)
        caps.append(RavenoxCapability.calendar.rawValue)
        caps.append(RavenoxCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(RavenoxCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            RavenoxScreenCommand.record.rawValue,
            RavenoxSystemCommand.notify.rawValue,
            RavenoxChatCommand.push.rawValue,
        ...
        if caps.contains(RavenoxCapability.camera.rawValue) {
        ...
        if caps.contains(RavenoxCapability.location.rawValue) {
            commands.append(RavenoxLocationCommand.get.rawValue)
        }
        if caps.contains(RavenoxCapability.device.rawValue) {
            commands.append(RavenoxDeviceCommand.status.rawValue)
            commands.append(RavenoxDeviceCommand.info.rawValue)
        }
        if caps.contains(RavenoxCapability.watch.rawValue) {
            commands.append(RavenoxWatchCommand.status.rawValue)
            commands.append(RavenoxWatchCommand.notify.rawValue)
        }
        if caps.contains(RavenoxCapability.photos.rawValue) {
            commands.append(RavenoxPhotosCommand.latest.rawValue)
        }
        if caps.contains(RavenoxCapability.contacts.rawValue) {
            commands.append(RavenoxContactsCommand.search.rawValue)
            commands.append(RavenoxContactsCommand.add.rawValue)
        }
        if caps.contains(RavenoxCapability.calendar.rawValue) {
            commands.append(RavenoxCalendarCommand.events.rawValue)
            commands.append(RavenoxCalendarCommand.add.rawValue)
        }
        if caps.contains(RavenoxCapability.reminders.rawValue) {
            commands.append(RavenoxRemindersCommand.list.rawValue)
            commands.append(RavenoxRemindersCommand.add.rawValue)
        }
        if caps.contains(RavenoxCapability.motion.rawValue) {
            commands.append(RavenoxMotionCommand.activity.rawValue)
            commands.append(RavenoxMotionCommand.pedometer.rawValue)
        }
