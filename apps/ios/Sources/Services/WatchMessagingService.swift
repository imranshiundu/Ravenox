...
enum WatchMessagingError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .watchAppNotInstalled:
            "WATCH_UNAVAILABLE: Ravenox watch companion app is not installed"
        }
    }
}

final class WatchMessagingService: NSObject, WatchMessagingServicing, @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.ravenox", category: "watch.messaging")
        ...
        priority: RavenoxNotificationPriority?) async throws -> WatchNotificationSendResult
    {
        ...
        let payload: [String: Any] = [
            ...
            "title": title,
            "body": body,
            "priority": priority?.rawValue ?? RavenoxNotificationPriority.active.rawValue,
