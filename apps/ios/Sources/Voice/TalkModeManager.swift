...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__RAVENOX_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<RavenoxTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = RavenoxTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> RavenoxTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return RavenoxTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> RavenoxTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> RavenoxTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return RavenoxTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> RavenoxTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = RavenoxTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: RavenoxTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: RavenoxAgentEventPayload.self) else {
                continue
            }
