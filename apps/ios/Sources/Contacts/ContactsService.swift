...
final class ContactsService: ContactsServicing {
    ...
    func search(params: RavenoxContactsSearchParams) async throws -> RavenoxContactsSearchPayload {
        ...
        return RavenoxContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: RavenoxContactsAddParams) async throws -> RavenoxContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return RavenoxContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return RavenoxContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> RavenoxContactPayload {
        RavenoxContactPayload(
