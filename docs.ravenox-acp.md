...
This document describes how the Ravenox ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

.ravenox acp` exposes an ACP agent over stdio and forwards prompts to a running
Ravenox Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Ravenox Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run .ravenox acp` over stdio.

Example config:

```bash
ravenox config set gateway.remote.url wss://gateway-host:18789
ravenox config set gateway.remote.token <token>
```

Example run:

```bash
ravenox acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
ravenox acp --session agent:main:main
ravenox acp --session agent:design:main
ravenox acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Ravenox ACP” to start a thread.

## Execution Model

- ACP client spawns .ravenox acp` and speaks ACP messages over stdio.
...
```bash
ravenox acp --session agent:main:main
ravenox acp --session-label "support inbox"
ravenox acp --reset-session
```
...
.ravenox acp` resolves the Gateway URL and auth from CLI flags or config:
