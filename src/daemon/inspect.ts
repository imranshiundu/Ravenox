...
function isRavenoxGatewayLaunchdService(label: string, contents: string): boolean {
...
function isRavenoxGatewayTaskName(name: string): boolean {
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isRavenoxGatewayTaskName(name)) {
        continue;
      }
