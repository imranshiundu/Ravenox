...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Ravenox onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: RavenoxConfig = snapshot.valid ? snapshot.config : {};
  ...
  let nextConfig: RavenoxConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
