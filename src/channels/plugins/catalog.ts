...
import { discoverRavenoxPlugins } from "../../plugins/discovery.js";
import type { RavenoxPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, RavenoxPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["RAVENOX_PLUGIN_CATALOG_PATHS", "RAVENOX_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<RavenoxPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: RavenoxPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: RavenoxPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverRavenoxPlugins({ workspaceDir: options.workspaceDir });
