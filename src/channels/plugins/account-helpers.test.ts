...
function cfg(accounts?: Record<string, unknown> | null): RavenoxConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as RavenoxConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as RavenoxConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as RavenoxConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as RavenoxConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as RavenoxConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as RavenoxConfig)).toBe("default");
    });
  });
});
