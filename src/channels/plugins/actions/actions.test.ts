...
function telegramCfg(): RavenoxConfig {
  return { channels: { telegram: { botToken: "tok" } } } as RavenoxConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as RavenoxConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as RavenoxConfig;
