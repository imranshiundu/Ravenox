...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub RAVENOX_STATE_DIR.
    // Also keep it outside os.tmpdir() so tmpdir localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["RAVENOX_STATE_DIR"]);
    process.env.RAVENOX_STATE_DIR = path.join(
...
describe("local media root guard", () => {
  ...
  it("allows default Ravenox state workspace and sandbox roots", async () => {
  ...
  it("rejects default Ravenox state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
