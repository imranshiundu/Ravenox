...
homepage: https://docs.ravenox.ai/automation/hooks#command-logger
metadata:
  {
    .ravenox":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Ravenox" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.ravenox/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ravenox/logs/commands.log ~/.ravenox/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d.ravenox`:
   ```
   /home/username/.ravenox/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
