...
This directory contains hooks that ship with Ravenox. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.ravenox.ravenox.json`:
...
2. Restart gateway: `pkill -9 -f .ravenox.*gateway' && pnpm.ravenox gateway`
3. Enable the hook: .ravenox hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.ravenox.ai/automation/hooks
