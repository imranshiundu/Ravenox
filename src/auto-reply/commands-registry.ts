...
export function isCommandEnabled(cfg: RavenoxConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: RavenoxConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as RavenoxConfig),
...
export function getCommandDetection(_cfg?: RavenoxConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: RavenoxConfig) {
