...
import { scheduleGatewaySigusr1Restart, triggerRavenoxRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Ravenox in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRavenoxRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Ravenox via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
