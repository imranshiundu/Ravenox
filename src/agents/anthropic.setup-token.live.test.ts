...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "RAVENOX_LIVE_SETUP_TOKEN did not look like a setup-token. Use RAVENOX_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set RAVENOX_LIVE_SETUP_TOKEN_VALUE or RAVENOX_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureRavenoxModelsJson(cfg, tokenSource.agentDir);
