...
const bundledPluginsDirSnapshot = captureEnv(["RAVENOX_BUNDLED_PLUGINS_DIR"]);
...
describe("createRavenoxCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    ...
    const envSnapshot = captureEnv(["RAVENOX_SHELL_ENV_TIMEOUT_MS"]);
    const result = await (async () => {
      try {
        process.env.RAVENOX_SHELL_ENV_TIMEOUT_MS = "1000";
