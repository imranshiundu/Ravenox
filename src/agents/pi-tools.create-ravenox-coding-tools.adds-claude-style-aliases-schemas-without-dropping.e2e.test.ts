...
describe("createRavenoxCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createRavenoxTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createRavenoxCodingTools({ messageProvider: "discord" });
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createRavenoxReadTool(
      baseRead as unknown as Parameters<typeof createRavenoxReadTool>[0],
    );
