...
function restoreRavenoxStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.RAVENOX_STATE_DIR;
    return;
  }
  process.env.RAVENOX_STATE_DIR = originalValue;
}

const originalStateDir = process.env.RAVENOX_STATE_DIR;

afterEach(() => {
  restoreRavenoxStateDir(originalStateDir);
});
