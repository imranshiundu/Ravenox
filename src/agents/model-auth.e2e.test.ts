...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), .ravenox-oauth-"));

    try {
      process.env.RAVENOX_STATE_DIR = tempDir;
      process.env.RAVENOX_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.RAVENOX_AGENT_DIR;
      ...
      const store = ensureAuthProfileStore(process.env.RAVENOX_AGENT_DIR, {
