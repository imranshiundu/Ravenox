...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".ravenox-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): RavenoxConfig {
  return {
    agents: {
      defaults: {
  ...
  it("allows workspace images via createRavenoxCodingTools default workspace root", async () => {
    await withTempWorkspacePng(async ({ imagePath }) => {
      ...
      try {
        ...
        const tools = createRavenoxCodingTools({ config: cfg, agentDir });
