...
import { resolveAgent AurthurAgentDir } from "./agent-paths.js";
import {
...
import { ensureAgent AurthurModelsJson } from "./models-config.js";
  ...
  it("uses the first github-copilot profile when env tokens are missing", async () => {
    await withTempHome(async (home) => {
      await withUnsetCopilotTokenEnv(async () => {
        ...
        await ensureAgent AurthurModelsJson({ models: { providers: {} } }, agentDir);
  ...
  it("does not override explicit github-copilot provider config", async () => {
    await withTempHome(async () => {
      await withCopilotGithubToken("gh-token", async () => {
        await ensureAgent AurthurModelsJson({
          models: {
            providers: {
              "github-copilot": {
        ...
        const agentDir = resolveAgent AurthurAgentDir();
