...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses RAVENOX_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      RAVENOX_HOME: "/srv.ravenox-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv.ravenox-home"), ".ravenox", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".ravenox", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace(.ravenox-workspace-");
