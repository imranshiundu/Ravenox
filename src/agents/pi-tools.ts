...
function resolveExecConfig(params: { cfg?: RavenoxConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: RavenoxConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createRavenoxCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createRavenoxReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
