...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.RAVENOX_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, "".ravenox", `workspace-${profile}`);
  }
  return path.join(home, "".ravenox", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = "".ravenox";
