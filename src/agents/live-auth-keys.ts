...
const GOOGLE_LIVE_SINGLE_KEY = "RAVENOX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "RAVENOX_LIVE_ANTHROPIC_KEY",
    listVar: "RAVENOX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "RAVENOX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `RAVENOX_LIVE_${base}_KEY`;
