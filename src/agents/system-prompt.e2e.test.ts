...
describe("buildAgentSystemPrompt", () => {
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Ravenox docs: /tmp()/docs");
    expect(prompt).toContain(
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp(). For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
