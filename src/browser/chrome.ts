...
export function resolveRavenoxUserDataDir(profileName = DEFAULT_RAVENOX_BROWSER_PROFILE_NAME) {
...
export async function launchRavenoxChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_RAVENOX_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateRavenoxProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
...
export async function stopRavenoxChrome(running: RunningChrome, timeoutMs = 2500) {
