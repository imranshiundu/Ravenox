...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    previousGatewayToken = process.env.RAVENOX_GATEWAY_TOKEN;
    process.env.RAVENOX_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  afterEach(async () => {
    ...
    if (previousGatewayToken === undefined) {
      ...
      process.env.RAVENOX_GATEWAY_TOKEN = previousGatewayToken;
    }
  });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    process.env.RAVENOX_GATEWAY_TOKEN = "test-gateway-token";
    try {
      cdpUrl = `http://127.0.0.1:${port}`;
      ...
      if (prev === undefined) {
        ...
        process.env.RAVENOX_GATEWAY_TOKEN = prev;
      }
