...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".ravenox-profile-decorated");
}
 ...
 */
export function decorateRavenoxProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_RAVENOX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_RAVENOX_BROWSER_COLOR).toUpperCase();
