import type { BrowserConfig, BrowserProfileConfig, RavenoxConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_RAVENOX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_RAVENOX_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_RAVENOX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_RAVENOX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Ravenox browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(
  ...
  rootConfig?: RavenoxConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_RAVENOX_BROWSER_ENABLED;
