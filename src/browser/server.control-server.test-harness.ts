...
vi.mock("./chrome.js", () => ({
  ...
  launchRavenoxChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.RAVENOX_GATEWAY_PORT;
    process.env.RAVENOX_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.RAVENOX_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.RAVENOX_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.RAVENOX_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.RAVENOX_GATEWAY_TOKEN;
    } else {
      process.env.RAVENOX_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.RAVENOX_GATEWAY_PASSWORD;
    } else {
      process.env.RAVENOX_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
