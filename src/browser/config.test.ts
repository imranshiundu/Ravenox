...
describe("browser config", () => {
  ...
  it("derives default ports from RAVENOX_GATEWAY_PORT when unset", () => {
    const prev = process.env.RAVENOX_GATEWAY_PORT;
    process.env.RAVENOX_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.RAVENOX_GATEWAY_PORT = prev;
      }
    }
  });
