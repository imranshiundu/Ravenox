...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.RAVENOX_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
function resolveRelayAuthToken(): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or RAVENOX_GATEWAY_TOKEN)",
  );
}
...
async function looksLikeRavenoxRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Ravenox-Extension-Relay",
          revision: "0",
          userAgent: "Ravenox-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Ravenox/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeRavenoxRelay(info.baseUrl))) {
