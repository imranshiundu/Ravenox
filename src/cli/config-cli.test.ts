...
import type { ConfigFileSnapshot, RavenoxConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<(cfg: RavenoxConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: RavenoxConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: RavenoxConfig, config: RavenoxConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: RavenoxConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
