...
export function resolveGitInstallDir(): string {
  const override = process.env.RAVENOX_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveRavenoxPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", RAVENOX_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`RAVENOX_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
