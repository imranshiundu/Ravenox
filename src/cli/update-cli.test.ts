...
describe("update-cli", () => {
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveRavenoxPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveRavenoxPackageRoot).mockReset();
    ...
    vi.mocked(resolveRavenoxPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Ravenox update status");
  });
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as RavenoxConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["RAVENOX_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.RAVENOX_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    try {
      setTty(true);
      process.env.RAVENOX_GIT_DIR = tempDir;
