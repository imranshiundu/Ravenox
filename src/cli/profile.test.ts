...
describe("applyCliProfileEnv", () => {
  ...
  it("uses RAVENOX_HOME when deriving profile state dir", () => {
    ...
    const resolvedHome = path.resolve("/srv()-home");
...
describe("formatCliCommand", () => {
  ...
  it("trims whitespace from profile", () => {
    expect(formatCliCommand("ravenox doctor --fix", { RAVENOX_PROFILE: "  j()  " })).toBe(
      "ravenox --profile j() doctor --fix",
    );
  });
  ...
  it("handles pnpm wrapper", () => {
    expect(formatCliCommand("pnpm() doctor", { RAVENOX_PROFILE: "work" })).toBe(
