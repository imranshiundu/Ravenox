...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.RAVENOX_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.RAVENOX_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.RAVENOX_TEST_TAILSCALE_BINARY;
    } else {
      process.env.RAVENOX_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
