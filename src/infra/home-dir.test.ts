...
describe("resolveEffectiveHomeDir", () => {
  ...
  it("expands RAVENOX_HOME when set to ~", () => {
    const env = {
      RAVENOX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for RAVENOX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { RAVENOX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when RAVENOX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ RAVENOX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
