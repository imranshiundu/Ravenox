...
const DEFAULT_PACKAGE_NAME = .ravenox";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), .ravenox-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, .ravenox.mjs");
    ...
    const doctorStep = await runStep(
      step(.ravenox doctor", doctorArgv, gitRoot, { RAVENOX_UPDATE_IN_PROGRESS: "1" }),
    );
