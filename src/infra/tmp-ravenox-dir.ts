...
export function resolvePreferredRavenoxTmpDir(
  options: ResolvePreferredRavenoxTmpDirOptions = {},
): string {
  ...
  try {
    const preferred = lstatSync(POSIX_RAVENOX_TMP_DIR);
    if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
      return fallback();
    }
    accessSync(POSIX_RAVENOX_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
    if (!isSecureDirForUser(preferred)) {
      return fallback();
    }
    return POSIX_RAVENOX_TMP_DIR;
  } catch (err) {
  ...
  try {
    ...
    mkdirSync(POSIX_RAVENOX_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_RAVENOX_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
