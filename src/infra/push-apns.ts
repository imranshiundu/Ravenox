...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.RAVENOX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.RAVENOX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set RAVENOX_APNS_TEAM_ID and RAVENOX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.RAVENOX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.RAVENOX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.RAVENOX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set RAVENOX_APNS_PRIVATE_KEY_P8 or RAVENOX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading RAVENOX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
