...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: RavenoxConfig | null = null;
  ...
  get(key: string, cfg: RavenoxConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: RavenoxConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: RavenoxConfig): void {
