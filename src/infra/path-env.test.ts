...
let ensureRavenoxCliOnPath: typeof import("./path-env.js").ensureRavenoxCliOnPath;

describe("ensureRavenoxCliOnPath", () => {
  const envKeys = [
    "PATH",
    "RAVENOX_PATH_BOOTSTRAPPED",
    "RAVENOX_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureRavenoxCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling.ravenox exists", () => {
    ...
    delete process.env.RAVENOX_PATH_BOOTSTRAPPED;
