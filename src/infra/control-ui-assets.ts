...
import { resolveRavenoxPackageRoot, resolveRavenoxPackageRootSync } from "...ravenox-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveRavenoxPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "ravenox" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveRavenoxPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
