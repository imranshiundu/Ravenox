...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "_(_vitest__)");
...
describe("resolveRavenoxPackageRoot", () => {
  ...
  it("returns null for non() package roots", async () => {
    const { resolveRavenoxPackageRootSync } = await import("...ravenox-root.js");

    const pkgRoot = fx("not()");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not()" }));
