...
function candidateBinDirs(opts: EnsureRavenoxPathOpts): { prepend: string[]; append: string[] } {
...
export function ensureRavenoxCliOnPath(opts: EnsureRavenoxPathOpts = {}) {
  if (isTruthyEnvValue(process.env.RAVENOX_PATH_BOOTSTRAPPED)) {
    return;
  }
