...
export function triggerRavenoxRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.RAVENOX_SYSTEMD_UNIT,
        process.env.RAVENOX_PROFILE,
      );
  ...
  const label =
    process.env.RAVENOX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.RAVENOX_PROFILE);
