...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.RAVENOX_TTS_PREFS;
      process.env.RAVENOX_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.RAVENOX_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: RavenoxConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
