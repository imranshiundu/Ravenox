...
describe("canvas host", () => {
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window(A2UI) = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/_(__)/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      ...
      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/_(__)/a2ui/a2ui.bundle.js`,
      );
