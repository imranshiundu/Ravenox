...
function summarizeGroupPolicy(cfg: RavenoxConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: RavenoxConfig): boolean {
...
function collectModels(cfg: RavenoxConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: RavenoxConfig): Set<string> {
  const baseCfg: RavenoxConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: RavenoxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: RavenoxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: RavenoxConfig): boolean {
...
function isBrowserEnabled(cfg: RavenoxConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: RavenoxConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
...
export function collectSecretsInConfigFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer RAVENOX_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectSandboxDockerNoopFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: RavenoxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
