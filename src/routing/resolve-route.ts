...
function listAgents(cfg: RavenoxConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: RavenoxConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: RavenoxConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<RavenoxConfig, EvaluatedBindingsCache>();
