...
describe("shared/frontmatter", () => {
  ...
  test("resolveRavenoxManifestBlock returns undefined for invalid input", () => {
    expect(resolveRavenoxManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveRavenoxManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveRavenoxManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
