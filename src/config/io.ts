...
import type { RavenoxConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareRavenoxVersions } from "./version.js";
...
function coerceConfig(value: unknown): RavenoxConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as RavenoxConfig;
}
...
function stampConfigVersion(cfg: RavenoxConfig): RavenoxConfig {
...
function warnIfConfigFromFuture(cfg: RavenoxConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareRavenoxVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Ravenox (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as RavenoxConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): RavenoxConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as RavenoxConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: RavenoxConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as RavenoxConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.RAVENOX_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.RAVENOX_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.RAVENOX_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.RAVENOX_WATCH_SESSION === "string" &&
        deps.env.RAVENOX_WATCH_SESSION.trim().length > 0
          ? deps.env.RAVENOX_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.RAVENOX_WATCH_COMMAND === "string" &&
        deps.env.RAVENOX_WATCH_COMMAND.trim().length > 0
          ? deps.env.RAVENOX_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `RAVENOX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.RAVENOX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.RAVENOX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): RavenoxConfig {
