...
describe("oauth paths", () => {
  it("prefers RAVENOX_OAUTH_DIR over RAVENOX_STATE_DIR", () => {
    const env = {
      RAVENOX_OAUTH_DIR: "/custom/oauth",
      RAVENOX_STATE_DIR: "/custom/state",
  ...
  it("derives oauth path from RAVENOX_STATE_DIR when unset", () => {
    const env = {
      RAVENOX_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  function expectRavenoxHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.RAVENOX_HOME;
    if (!configuredHome) {
