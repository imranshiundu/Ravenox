...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from RAVENOX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { RAVENOX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.RAVENOX_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent paths when RAVENOX_STATE_DIR differs from stored paths", () => {
    const prev = process.env.RAVENOX_STATE_DIR;
    process.env.RAVENOX_STATE_DIR = path.resolve("/different/state");
    try {
