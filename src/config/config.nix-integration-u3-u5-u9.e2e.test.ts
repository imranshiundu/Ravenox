...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when RAVENOX_NIX_MODE is not set", () => {
      expect(resolveIsNixMode(envWith({ RAVENOX_NIX_MODE: undefined }))).toBe(false);
    });

    it("isNixMode is false when RAVENOX_NIX_MODE is empty", () => {
      expect(resolveIsNixMode(envWith({ RAVENOX_NIX_MODE: "" }))).toBe(false);
    });

    it("isNixMode is false when RAVENOX_NIX_MODE is not '1'", () => {
      expect(resolveIsNixMode(envWith({ RAVENOX_NIX_MODE: "true" }))).toBe(false);
    });

    it("isNixMode is true when RAVENOX_NIX_MODE=1", () => {
      expect(resolveIsNixMode(envWith({ RAVENOX_NIX_MODE: "1" }))).toBe(true);
    });
  });
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("STATE_DIR respects RAVENOX_STATE_DIR override", () => {
      expect(resolveStateDir(envWith({ RAVENOX_STATE_DIR: "/custom/state/dir" }))).toBe(
        path.resolve("/custom/state/dir"),
      );
    });

    it("STATE_DIR respects RAVENOX_HOME when state override is unset", () => {
    ...
    it("CONFIG_PATH defaults to RAVENOX_HOME/.ravenox().json", () => {
      ...
      expect(
        resolveConfigPathCandidate(
          envWith({
            RAVENOX_HOME: customHome,
            RAVENOX_CONFIG_PATH: undefined,
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", () => {
      expect(resolveGatewayPort({}, envWith({ RAVENOX_GATEWAY_PORT: undefined }))).toBe(
        DEFAULT_GATEWAY_PORT,
      );
    });

    it("prefers RAVENOX_GATEWAY_PORT over config", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19002 } },
          envWith({ RAVENOX_GATEWAY_PORT: "19001" }),
        ),
      ).toBe(19001);
    });
    ...
    it("falls back to config when env is invalid", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19003 } },
          envWith({ RAVENOX_GATEWAY_PORT: "nope" }),
        ),
      ).toBe(19003);
    });
  });
