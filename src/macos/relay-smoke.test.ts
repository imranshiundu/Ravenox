...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { RAVENOX_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { RAVENOX_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports RAVENOX_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { RAVENOX_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { RAVENOX_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { RAVENOX_SMOKE: "nope" })).toBe("qr");
  });
});
