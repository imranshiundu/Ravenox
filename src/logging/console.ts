...
type ConsoleConfigLoader = () => RavenoxConfig["logging"] | undefined;
const loadConfigFallbackDefault: ConsoleConfigLoader = () => {
  try {
    const loaded = requireConfig("../config/config.js") as {
      loadConfig?: () => RavenoxConfig;
    };
...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && process.env.RAVENOX_TEST_CONSOLE !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  let cfg: RavenoxConfig["logging"] | undefined =
