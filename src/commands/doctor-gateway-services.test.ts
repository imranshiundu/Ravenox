...
async function runRepair(cfg: RavenoxConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      RAVENOX_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service RAVENOX_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      RAVENOX_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses RAVENOX_GATEWAY_TOKEN when config token is missing", async () => {
    const previousToken = process.env.RAVENOX_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.RAVENOX_GATEWAY_TOKEN = previousToken;
      }
    }
  });
});
