...
export function resolveTargets(cfg: RavenoxConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.RAVENOX_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.RAVENOX_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: RavenoxConfig) {
