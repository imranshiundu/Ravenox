...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.RAVENOX_PROFILE);
    hints.push(
      `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`,
    );
    hints.push(`Then reinstall: ${formatCliCommand(.ravenox gateway install", env)}`);
  }
  ...
  if (runtime.status === "stopped") {
    ...
    if (platform === "darwin") {
      ...
      const unit = resolveGatewaySystemdServiceName(env.RAVENOX_PROFILE);
      hints.push(`Logs: journalctl --user -u ${unit}.service -n 200 --no-pager`);
    } else if (platform === "win32") {
      const task = resolveGatewayWindowsTaskName(env.RAVENOX_PROFILE);
