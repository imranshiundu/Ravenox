...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, "".ravenox", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: RavenoxConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`RAVENOX_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["RAVENOX_GATEWAY_TOKEN", await getenv("RAVENOX_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["RAVENOX_GATEWAY_PASSWORD", await getenv("RAVENOX_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "RAVENOX_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use RAVENOX_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> RAVENOX_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
