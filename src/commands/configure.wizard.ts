...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.RAVENOX_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.RAVENOX_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Ravenox update wizard" : "Ravenox configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: RavenoxConfig = snapshot.valid ? snapshot.config : {};
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.RAVENOX_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.RAVENOX_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.RAVENOX_GATEWAY_TOKEN;
