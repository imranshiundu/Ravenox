...
export async function loadValidConfigOrThrow(): Promise<RavenoxConfig> {
...
export async function updateConfig(
  mutator: (cfg: RavenoxConfig) => RavenoxConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: RavenoxConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: RavenoxConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("ravenox agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
