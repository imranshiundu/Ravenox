...
function resolveAgentName(cfg: RavenoxConfig, agentId: string) {
...
function resolveAgentModel(cfg: RavenoxConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: RavenoxConfig): AgentSummary[] {
