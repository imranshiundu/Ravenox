...
function stripUnknownConfigKeys(config: RavenoxConfig): {
  config: RavenoxConfig;
  removed: string[];
} {
  const parsed = RavenoxSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: RavenoxConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: RavenoxConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: RavenoxConfig): Promise<{
  config: RavenoxConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: RavenoxConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: RavenoxConfig): {
  config: RavenoxConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: RavenoxConfig): {
  config: RavenoxConfig;
  changes: string[];
} {
