...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: RavenoxPluginDefinition;
  register?: RavenoxPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as RavenoxPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as RavenoxPluginDefinition;
...
export function loadRavenoxPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverRavenoxPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: RavenoxPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as RavenoxPluginModule;
    } catch (err) {
