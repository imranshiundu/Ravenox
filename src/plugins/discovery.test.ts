...
import { discoverRavenoxPlugins } from "./discovery.js";
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  ...
  const prevBundled = process.env.RAVENOX_BUNDLED_PLUGINS_DIR;
  process.env.RAVENOX_STATE_DIR = stateDir;
  process.env.RAVENOX_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  try {
    return await fn();
  } finally {
    if (prev === undefined) {
      ...
      process.env.RAVENOX_STATE_DIR = prev;
    }
    if (prevBundled === undefined) {
      delete process.env.RAVENOX_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.RAVENOX_BUNDLED_PLUGINS_DIR = prevBundled;
    }
  }
}
...
describe("discoverRavenoxPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverRavenoxPlugins({ workspaceDir });
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverRavenoxPlugins({ extraPaths: [packDir] });
    });
