...
import { loadRavenoxPlugins } from "./loader.js";
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.RAVENOX_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadRavenoxPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.RAVENOX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadRavenoxPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadRavenoxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadRavenoxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
