...
describe("docker-setup.sh", () => {
  ...
  it("rejects injected multiline RAVENOX_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.stderr).toContain("RAVENOX_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid RAVENOX_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        RAVENOX_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid RAVENOX_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("RAVENOX_HOME_VOLUME must match");
  });
