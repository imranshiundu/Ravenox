...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["RAVENOX_BASE_ENV"]);
    process.env.RAVENOX_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.RAVENOX_BASE_ENV ?? "") + "|" + (process.env.RAVENOX_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { RAVENOX_TEST_ENV: "ok" },
        },
      );
