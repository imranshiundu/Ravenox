...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.RAVENOX_STATE_DIR;
  previousConfigPath = process.env.RAVENOX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.RAVENOX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.RAVENOX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.RAVENOX_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.RAVENOX_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.RAVENOX_SKIP_CHANNELS;
...
function applyGatewaySkipEnv() {
  ...
  process.env.RAVENOX_SKIP_PROVIDERS = "1";
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.RAVENOX_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.RAVENOX_CONFIG_PATH;
    } else {
      process.env.RAVENOX_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.RAVENOX_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.RAVENOX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.RAVENOX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.RAVENOX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.RAVENOX_SKIP_CANVAS_HOST;
    } else {
      process.env.RAVENOX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.RAVENOX_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.RAVENOX_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.RAVENOX_SKIP_CHANNELS;
    } else {
      process.env.RAVENOX_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.RAVENOX_SKIP_PROVIDERS;
    } else {
      process.env.RAVENOX_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.RAVENOX_SKIP_CRON;
    } else {
      process.env.RAVENOX_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.RAVENOX_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.RAVENOX_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["RAVENOX_GATEWAY_TOKEN"]);
  const prev = process.env.RAVENOX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.RAVENOX_GATEWAY_TOKEN = fallbackToken;
  }
