...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: RavenoxConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: RavenoxConfig) {
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          RAVENOX_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
