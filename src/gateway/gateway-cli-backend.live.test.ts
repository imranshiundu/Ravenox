...
const CLI_LIVE = isTruthyEnvValue(process.env.RAVENOX_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.RAVENOX_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.RAVENOX_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("RAVENOX_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.RAVENOX_CONFIG_PATH,
      token: process.env.RAVENOX_GATEWAY_TOKEN,
      skipChannels: process.env.RAVENOX_SKIP_CHANNELS,
      skipGmail: process.env.RAVENOX_SKIP_GMAIL_WATCHER,
      skipCron: process.env.RAVENOX_SKIP_CRON,
      skipCanvas: process.env.RAVENOX_SKIP_CANVAS_HOST,
    ...
    process.env.RAVENOX_SKIP_CANVAS_HOST = "1";
    ...
    const rawModel = process.env.RAVENOX_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `RAVENOX_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.RAVENOX_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `RAVENOX_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "RAVENOX_LIVE_CLI_BACKEND_ARGS",
        process.env.RAVENOX_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`RAVENOX_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "RAVENOX_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.RAVENOX_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.RAVENOX_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.RAVENOX_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "RAVENOX_LIVE_CLI_BACKEND_IMAGE_MODE requires RAVENOX_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        ...
        process.env.RAVENOX_CONFIG_PATH = previous.configPath;
      }
      if (previous.token === undefined) {
        delete process.env.RAVENOX_GATEWAY_TOKEN;
      } else {
        process.env.RAVENOX_GATEWAY_TOKEN = previous.token;
      }
      if (previous.skipChannels === undefined) {
        delete process.env.RAVENOX_SKIP_CHANNELS;
      } else {
        process.env.RAVENOX_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.RAVENOX_SKIP_GMAIL_WATCHER;
      } else {
        process.env.RAVENOX_SKIP_GMAIL_WATCHER = previous.skipGmail;
      }
      if (previous.skipCron === undefined) {
        delete process.env.RAVENOX_SKIP_CRON;
      } else {
        process.env.RAVENOX_SKIP_CRON = previous.skipCron;
      }
      if (previous.skipCanvas === undefined) {
        delete process.env.RAVENOX_SKIP_CANVAS_HOST;
      } else {
        process.env.RAVENOX_SKIP_CANVAS_HOST = previous.skipCanvas;
      }
