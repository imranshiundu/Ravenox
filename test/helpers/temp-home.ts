...
function snapshotEnv(): EnvSnapshot {
  return {
   ...
   .ravenoxHome: process.env.RAVENOX_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("RAVENOX_HOME", snapshot.ravenoxHome);
  restoreKey("RAVENOX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked RAVENOX_HOME.
  delete process.env.RAVENOX_HOME;
  process.env.RAVENOX_STATE_DIR = path.join(base, ".ravenox");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? .ravenox-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ravenox", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
