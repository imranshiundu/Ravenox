...
import type { RavenoxConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir(.ravenox-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir(.ravenox-bin-sherpa-");
      const modelDir = await makeTempDir(.ravenox-sherpa-model-");
      ...
      const cfg: RavenoxConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir(.ravenox-bin-whispercpp-");
      const modelDir = await makeTempDir(.ravenox-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir(.ravenox-bin-gemini-");
      ...
      const cfg: RavenoxConfig = { tools: { media: { image: {} } } };
